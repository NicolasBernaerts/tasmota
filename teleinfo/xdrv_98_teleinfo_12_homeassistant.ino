/*
  xdrv_98_teleinfo_12_homeassistant.ino - Home assistant auto-discovery integration for Teleinfo
...
static const char PSTR_HASS_PUB_CALENDAR_TODAY[]   PROGMEM = "Couleur Aujourdhui"    "|CONTRACT_TDAY"   "|.CONTRACT.tday"   "|calendar" "|" "|" "|";
static const char PSTR_HASS_PUB_CALENDAR_TOMRW[]   PROGMEM = "Couleur Demain"        "|CONTRACT_TMRW"   "|.CONTRACT.tmrw"   "|calendar" "|" "|" "|";
...
// Commands
static const char kTeleinfoHassCommands[]  PROGMEM =   "|"    "hass";
void (* const TeleinfoHassCommand[])(void) PROGMEM = { &CmndTeleinfoHassEnable };
...
// load configuration
void TeleinfoHassLoadConfig () 
{
  teleinfo_hass.enabled = Settings->rf_code[16][0];
}

// save configuration
void TeleinfoHassSaveConfig () 
{
  Settings->rf_code[16][0] = teleinfo_hass.enabled;
}
...
// set integration
void TeleinfoHassSet (const bool enabled) 
{
  // update status
  teleinfo_hass.enabled = enabled;

  // reset publication flags
  teleinfo_hass.stage     = TIC_PUB_CONNECT; 
  teleinfo_hass.sub_stage = 0; 

  // save configuration
  TeleinfoHassSaveConfig ();
}

// get integration
bool TeleinfoHassGet () 
{
  return (teleinfo_hass.enabled == 1);
}
...
void CmndTeleinfoHassEnable ()
{
  if (XdrvMailbox.data_len > 0) TeleinfoHassSet (XdrvMailbox.payload != 0);
...
void TeleinfoHassInit ()
{
  // load config
  TeleinfoHassLoadConfig ();

  // log help command
  AddLog (LOG_LEVEL_INFO, PSTR ("HLP: Run hass to set Home Assistant auto-discovery [%u]"), teleinfo_hass.enabled);
}

// trigger publication
void TeleinfoHassEvery250ms ()
{
  // if on battery, no publication of persistent data
  if (TeleinfoDriverIsOnBattery ()) teleinfo_hass.stage = UINT8_MAX;

  // if already published or running on battery, ignore 
  if (!teleinfo_hass.enabled) return;
  if (teleinfo_hass.stage == UINT8_MAX) return;
  if (!RtcTime.valid) return;
  if (!MqttIsConnected ()) return;

  // publication
  TeleinfoHassPublish (teleinfo_hass.stage, teleinfo_hass.sub_stage);

  // handle increment
  if ((teleinfo_hass.stage == TIC_PUB_RELAY_DATA) || (teleinfo_hass.stage == TIC_PUB_TOTAL_INDEX)) teleinfo_hass.sub_stage++;
    else teleinfo_hass.stage++;
  if ((teleinfo_hass.stage == TIC_PUB_RELAY_DATA)  && (teleinfo_hass.sub_stage >= 8)) teleinfo_hass.stage++;
  if ((teleinfo_hass.stage == TIC_PUB_TOTAL_INDEX) && (teleinfo_hass.sub_stage >= teleinfo_contract.period_qty)) teleinfo_hass.stage++;
  if ((teleinfo_hass.stage != TIC_PUB_RELAY_DATA)  && (teleinfo_hass.stage != TIC_PUB_TOTAL_INDEX)) teleinfo_hass.sub_stage = 0; 

  // increment to next stage
  if ((teleinfo_hass.stage == TIC_PUB_PROD)        && (!teleinfo_config.meter))       teleinfo_hass.stage = TIC_PUB_RELAY;
  if ((teleinfo_hass.stage == TIC_PUB_PROD)        && (teleinfo_prod.total_wh == 0))  teleinfo_hass.stage = TIC_PUB_CONSO;
  if ((teleinfo_hass.stage == TIC_PUB_CONSO)       && (teleinfo_conso.total_wh == 0)) teleinfo_hass.stage = TIC_PUB_RELAY;
  if ((teleinfo_hass.stage == TIC_PUB_PH1)         && (teleinfo_contract.phase == 1)) teleinfo_hass.stage = TIC_PUB_RELAY;
  if ((teleinfo_hass.stage == TIC_PUB_RELAY)       && (!teleinfo_config.relay))       teleinfo_hass.stage = TIC_PUB_TOTAL;
  if ((teleinfo_hass.stage == TIC_PUB_TOTAL)       && (!teleinfo_config.meter))       teleinfo_hass.stage = TIC_PUB_TOTAL_INDEX + 1;
  if ((teleinfo_hass.stage == TIC_PUB_TOTAL_PROD)  && (teleinfo_prod.total_wh == 0))  teleinfo_hass.stage = TIC_PUB_TOTAL_CONSO;
  if ((teleinfo_hass.stage == TIC_PUB_TOTAL_CONSO) && (teleinfo_conso.total_wh == 0)) teleinfo_hass.stage = TIC_PUB_TOTAL_INDEX + 1;

#ifdef USE_RTE_SERVER
  if ((teleinfo_hass.stage == TIC_PUB_RTE_TEMPO_TODAY)  && !TeleinfoRteTempoIsEnabled ())  teleinfo_hass.stage = TIC_PUB_RTE_POINTE_TODAY;
  if ((teleinfo_hass.stage == TIC_PUB_RTE_POINTE_TODAY) && !TeleinfoRtePointeIsEnabled ()) teleinfo_hass.stage = TIC_PUB_RTE_POINTE_TOMRW + 1;
#endif    // USE_RTE_SERVER

  if (teleinfo_hass.stage == TIC_PUB_MAX) teleinfo_hass.stage = UINT8_MAX;
}
...
void TeleinfoHassPublish (const uint8_t stage, const uint8_t index)
{
  uint32_t    ip_address;
...
#ifdef USE_RTE_SERVER
    case TIC_PUB_RTE_TEMPO_TODAY:  if (TeleinfoRteTempoIsEnabled ())  pstr_param = PSTR_HASS_PUB_RTE_TEMPO_TODAY;  break;
    case TIC_PUB_RTE_TEMPO_TOMRW:  if (TeleinfoRteTempoIsEnabled ())  pstr_param = PSTR_HASS_PUB_RTE_TEMPO_TOMRW;  break;
    case TIC_PUB_RTE_POINTE_TODAY: if (TeleinfoRtePointeIsEnabled ()) pstr_param = PSTR_HASS_PUB_RTE_POINTE_TODAY; break;
    case TIC_PUB_RTE_POINTE_TOMRW: if (TeleinfoRtePointeIsEnabled ()) pstr_param = PSTR_HASS_PUB_RTE_POINTE_TOMRW; break;
#endif  // RTE_SERVER
  ...
  if (pstr_param != nullptr)
  {
    ...
    if (teleinfo_hass.stage == TIC_PUB_CONTRACT_NAME) 
    {
      // get IP address
