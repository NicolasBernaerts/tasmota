...
void HassIntegrationEvery250ms ()
{
  ...
  if ((hass_integration.stage == TIC_PUB_TOTAL)       && (!teleinfo_config.meter))       hass_integration.stage = TIC_PUB_TOTAL_INDEX + 1;
  if ((hass_integration.stage == TIC_PUB_TOTAL_PROD)  && (teleinfo_prod.total_wh == 0))  hass_integration.stage = TIC_PUB_TOTAL_CONSO;
  if ((hass_integration.stage == TIC_PUB_TOTAL_CONSO) && (teleinfo_conso.total_wh == 0)) hass_integration.stage = TIC_PUB_TOTAL_INDEX + 1;

#ifdef USE_RTE_SERVER
  if ((hass_integration.stage == TIC_PUB_RTE_TEMPO_TODAY)  && !RteTempoIsEnabled ())  hass_integration.stage = TIC_PUB_RTE_POINTE_TODAY;
  if ((hass_integration.stage == TIC_PUB_RTE_POINTE_TODAY) && !RtePointeIsEnabled ()) hass_integration.stage = TIC_PUB_RTE_POINTE_TOMRW + 1;
...
#ifdef USE_RTE_SERVER
    case TIC_PUB_RTE_TEMPO_TODAY:  if (RteTempoIsEnabled ())  pstr_param = PSTR_HASS_PUB_RTE_TEMPO_TODAY;  break;
    case TIC_PUB_RTE_TEMPO_TOMRW:  if (RteTempoIsEnabled ())  pstr_param = PSTR_HASS_PUB_RTE_TEMPO_TOMRW;  break;
    case TIC_PUB_RTE_POINTE_TODAY: if (RtePointeIsEnabled ()) pstr_param = PSTR_HASS_PUB_RTE_POINTE_TODAY; break;
    case TIC_PUB_RTE_POINTE_TOMRW: if (RtePointeIsEnabled ()) pstr_param = PSTR_HASS_PUB_RTE_POINTE_TOMRW; break;
#endif  // RTE_SERVER
