...
bool TeleinfoEnergyHandleCommand ()
{
  ...
  if (serviced) 
  {
    ...
    if (XdrvMailbox.data_len == 0) 
    {
      ...
      AddLog (LOG_LEVEL_INFO, PSTR ("  display=%u      affichage sur page acceuil"), teleinfo_config.display);
...
uint8_t TeleinfoEnergyExecuteCommand (const int command, const long value)
{
  ...
  switch (command)
  {
    ...
    case TIC_CMND_DISPLAY:
      to_save = ((value < 2) && (teleinfo_config.display != (uint8_t)value));
      if (to_save) teleinfo_config.display = (uint8_t)value;
...
void TeleinfoConfigLoad () 
{
  ...
  teleinfo_config.display      = min ((uint)~Settings->teleinfo.display, 1U);
  teleinfo_config.meter        = min ((uint)~Settings->teleinfo.meter, 1U);
  teleinfo_config.policy       = min ((uint)Settings->teleinfo.policy, (uint)(TIC_POLICY_MAX - 1));
  teleinfo_config.calendar     = min ((uint)Settings->teleinfo.calendar, 1U);
  teleinfo_config.relay        = min ((uint)Settings->teleinfo.relay, 1U);
  teleinfo_config.tic          = min ((uint)Settings->teleinfo.tic, 1U);
...
void TeleinfoEnergyConfigSave (const bool save2rom) 
{
  ...
  Settings->teleinfo.display    = ~teleinfo_config.display;
