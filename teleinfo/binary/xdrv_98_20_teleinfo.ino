...
void TeleinfoDriverWebPageConfigure ()
{
  ...
  if (Webserver->hasArg (F ("save")))
  {
    ...
    if (Webserver->hasArg (F (CMND_TIC_ENERGY))) teleinfo_config.energy = 1;
      else teleinfo_config.energy = 0;

    // parameter 'meter' : set METER, PROD & CONTRACT section diffusion flag
    if (Webserver->hasArg (F (CMND_TIC_METER))) teleinfo_config.meter = 1;
      else teleinfo_config.meter = 0;

    // parameter 'calendar' : set CALENDAR section diffusion flag
    if (Webserver->hasArg (F (CMND_TIC_CALENDAR))) teleinfo_config.calendar = 1;
      else teleinfo_config.calendar = 0;

    // parameter 'relay' : set RELAY section diffusion flag
    if (Webserver->hasArg (F (CMND_TIC_RELAY))) teleinfo_config.relay = 1;
      else teleinfo_config.relay = 0;

    // parameter 'live' : set LIVE topic diffusion flag
    if (Webserver->hasArg (F (CMND_TIC_LIVE))) teleinfo_config.live = 1;
      else teleinfo_config.live = 0;

    // parameter 'tic' : set TIC topic diffusion flag
    if (Webserver->hasArg (F (CMND_TIC_TIC))) teleinfo_config.tic = 1;
    ...
    if (Webserver->hasArg (F (CMND_TIC_PERIOD))) teleinfo_config.led_period = 1;
      else teleinfo_config.led_period = 0;
  ...
  WSContentSend_P (PSTR ("<p class='dat'><span class='hval'>Publier 1 trame sur</span><span class='val'><input type='number' name='%s' min=1 max=7 step=1 value=%u></span><span class='uni'></span></p>\n"), PSTR (CMND_TIC_SKIP), teleinfo_config.skip);
  WSContentSend_P (PSTR ("</fieldset>\n"));
...
void TeleinfoDriverWebSensor ()
{
  ...
  // check if disabled
  if (teleinfo_config.display == 0) return;
...
bool Xdrv98 (const uint32_t function)
{
  ...
  if (!result) result = XdrvTeleinfoGraph (function);
#ifdef USE_UFILESYS
  if (!result) result = XdrvTeleinfoHisto (function);
#endif  // USE_UFILESYS

#ifdef ESP32
  if (!result) result = XdrvTeleinfoInfluxDB (function);
  if (!result) result = XdrvTeleinfoRte (function);
