...
struct {
  ...
  uint8_t      display   = 0;       // flag set if winky is displayed in main mage
...
void CmndTeleinfoWinkyHelp ()
{
  ...
  AddLog (LOG_LEVEL_INFO, PSTR (" - winky_start <volt>  = tension min. démarrage [%u.%02u]"), teleinfo_winky.volt.start / 1000, teleinfo_winky.volt.start % 1000 / 100);
  AddLog (LOG_LEVEL_INFO, PSTR (" - winky_stop <volt>   = tension min. arrêt [%u.%02u]"), teleinfo_winky.volt.stop / 1000, teleinfo_winky.volt.stop % 1000 / 100);
  AddLog (LOG_LEVEL_INFO, PSTR (" - winky_ref <farad>   = capacité super capa (F) [%u.%02u]"), teleinfo_winky.farad.ref / 1000, teleinfo_winky.farad.ref % 1000 / 100);
...
void CmndTeleinfoWinkyDisplay ()
{
  ...
  {
    // if new start voltage greater than stop voltage, set and save
    teleinfo_winky.display    = (uint8_t)XdrvMailbox.payload;
    Settings->knx_GA_param[0] = ~teleinfo_winky.display;
    SettingsSave (0);
  }
...
void TeleinfoWinkyLoadConfiguration () 
{
  ...
  // display
  teleinfo_winky.display = max (1, ~Settings->knx_GA_param[0]);
...
void TeleinfoWinkyWebSensor ()
{
  ...
  if (teleinfo_winky.enabled == 0) return;
  if (teleinfo_winky.display == 0) return;
