/*
  xdrv_15_teleinfo.ino - France Teleinfo energy sensor support for Tasmota devices
...
#define D_CMND_TELEINFO_DISPLAY         "display"
...
void TeleinfoDriverPublishTrigger ()
{
  ...
  // ask for all defined domoticz publications
  TeleinfoDomoticzPublishTrigger ();
...
#ifdef USE_TELEINFO_DOMOTICZ
  if (TeleinfoDomoticzPublishNeeded ()) TeleinfoDomoticzPublish ();
  else
#endif    // USE_TELEINFO_DOMOTICZ
...
void TeleinfoDriverWebSensor ()
{
  ...
  else
  {
    ...
    if (teleinfo_meter.display || (teleinfo_meter.nb_reset > 0) || (value >= 100))
    {
      WSContentSend_P (PSTR ("<div style='display:flex;padding:2px 0px;'>\n"));
