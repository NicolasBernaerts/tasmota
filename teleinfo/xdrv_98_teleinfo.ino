/*
  ...
  Version history :
    ...
    17/04/2026 v15.2  - Remove baudrate auto-detect
                        Switch ti historique mode by default
                        Add SOLAR and FORECAST to Live data
...
void TeleinfoDriverLedSwitch (const uint8_t status, const bool target)
{
  ...
  else
  {
    ...
    switch (status)
    {
      ...
      default: if (teleinfo_prod.papp > 0) level = TIC_LEVEL_PROD; else level = TeleinfoContractGetPeriodLevel (); break;
    }
...
void TeleinfoDriverLoadSettings ()
{
  ...
  teleinfo_config.baudrate     = (uint32_t)Settings->rf_code[15][8] * 300;
...
void TeleinfoDriverSaveSettings () 
{
  ...
  Settings->rf_code[15][8]         = (uint8_t)(teleinfo_config.baudrate / 300);
...
uint8_t TeleinfoDriverCalendarGetHP (const uint8_t day, const uint8_t slot)
{
  ...
  if (teleinfo_calendar[day].level > TIC_LEVEL_NONE) hchp = TeleinfoContractGetPeriodHP (teleinfo_calendar[day].arr_slot[slot]);
...
uint8_t TeleinfoDriverCalendarGetLevel (const uint8_t day, uint8_t slot, const bool use_rte)
{
  ...
  level = TeleinfoContractGetPeriodLevel (teleinfo_calendar[day].arr_slot[slot]);
...
void TeleinfoDriverAppendMeterSensor ()
{
  ...
  // if meter in production mode, publish complete production data
  if (teleinfo_prod.enabled)
  {
    // prod : global values
    ResponseAppend_P (PSTR (",\"PP\":%d"), teleinfo_prod.papp);
  ...
  // prodcution apparent power (allow to track excess of self production)
  else if (teleinfo_prod.cacsi) ResponseAppend_P (PSTR (",\"PP\":%d"), teleinfo_prod.papp);
...
void TeleinfoDriverAppendContractSensor ()
{
  ...
  // contract period
  TeleinfoContractGetPeriodLabel (str_value, sizeof (str_value));
  ...
  {
    TeleinfoContractGetPeriodCode (str_period, sizeof (str_period), index);
...
void TeleinfoDriverAppendRelaySensor ()
{
  ...
  if (teleinfo_conso.enabled)
    for (index = 0; index < teleinfo_contract_db.period_qty; index ++) 
    {
      ...
      level  = TeleinfoContractGetPeriodLevel (index);
      hchp   = TeleinfoContractGetPeriodHP (index);
...
void TeleinfoDriverAppendCalendarSensor ()
{
  ...
  MiscOptionPrepareJsonSection ();
  ResponseAppend_P (PSTR ("\"CAL\":{\"level\":%u,\"hp\":%u}"), TeleinfoContractGetPeriodLevel (), TeleinfoContractGetPeriodHP ());
}
...
void TeleinfoCalendarSetDemain (const char* pstr_color)
{
  ...
  // detect color
  level = TeleinfoContractDetectPeriodLevel (pstr_color);
...
void TeleinfoDriverInit ()
{
  ...
  // set web refresh to 1 sec (linky ready)
...
void TeleinfoDriverJsonAppend ()
{
  ...
  // if needed, remove ENERGY and append data
...
//  WSContentSend_P (PSTR ("<p class='dat'><input type='radio' name='%s' value='%d' %s>%s</p>\n"), PSTR (CMND_TIC_RATE), 115200, str_select, PSTR ("Auto-détection"));
  ...
  // 1200 bauds
  if (teleinfo_config.baudrate == 1200) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  WSContentSend_P (PSTR ("<p class='dat'><input type='radio' name='%s' value='%d' %s>%s</p>\n"), PSTR (CMND_TIC_RATE), 1200, str_select, PSTR ("Mode Historique"));

  // 9600 bauds
  if (teleinfo_config.baudrate == 9600) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  WSContentSend_P (PSTR ("<p class='dat'><input type='radio' name='%s' value='%d' %s>%s</p>\n"), PSTR (CMND_TIC_RATE), 9600, str_select, PSTR ("Mode Standard"));

  // other specific rate
  if ((teleinfo_config.baudrate != 1200) && (teleinfo_config.baudrate != 9600)) 
    WSContentSend_P (PSTR ("<p class='dat'><input type='radio' name='%s' value='%d' checked>Spécifique <small>(%u bauds)</small></p>\n"), PSTR (CMND_TIC_RATE), teleinfo_config.baudrate, teleinfo_config.baudrate);
  ...
  // prometheus
  pstr_title = PSTR ("API disponible sous /api/prometheus/metrics");
  WSContentSend_P (PSTR ("<p class='dat' title='%s'><input type='checkbox' name='%s' %s><label for='%s'>%s</label></p>\n"), pstr_title, PSTR ("prometheus"), PSTR ("checked disabled"), PSTR ("prometheus"), PSTR ("Prometheus"));

#ifdef USE_TELEINFO_HOMIE
  // Homie integration (auto-discovery)
  actual = TeleinfoHomieGet ();
  if (actual) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  pstr_title = PSTR ("Topic homie/... pour auto-découverte et la publication des données par Homie (OpenHab)");
  ...
  actual = TeleinfoHomeAssistantGet ();
  if (actual) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  pstr_title = PSTR ("Topic homeassistant/sensor/... pour auto-découverte par Home Assistant. Home Assistant exploitera alors le topic SENSOR");
  ...
  actual = TeleinfoThingsboardGet ();
  if (actual) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  pstr_title = PSTR ("Topic v1/telemetry/device/me pour la publication des données vers Thingsboard");
  ...
  actual = TeleinfoDomoticzGet ();
  if (actual) strcpy_P (str_select, PSTR ("checked")); else str_select[0] = 0;
  pstr_title = PSTR ("Topic domoticz/in pour la publication des données vers Domoticz");
...
// speed indocator
void TeleinfoDriverWebStatusRight ()
{
  // speed indicator
  WSContentSend_P (PSTR ("<span class='top"));
  if (!TeleinfoEnergySerialIsStarted ()) WSContentSend_P (PSTR (" disabled"));
    else if (teleinfo_meter.nb_message == 0) WSContentSend_P (PSTR (" ko"));
    else WSContentSend_P (PSTR (" ok"));
  WSContentSend_P (PSTR ("'>%u</span>\n"), teleinfo_config.baudrate);
}
...
//  if (teleinfo_config.led) for (index = 0; index < LST_MAX; index++) Web.slider[index] = teleinfo_led.arr_slider[index];
  ...
  if (teleinfo_meter.nb_message > 0)
  {
    ...
    if (level == TIC_LED_STEP_OK) GetTextIndexed (str_text, sizeof (str_text), TeleinfoContractGetPeriodLevel (), kTeleinfoLevelIcon);
      else str_text[0] = 0;
    ...
    if (teleinfo_config.arr_main[TIC_DISPLAY_CONTRACT])
    {
      ...
      for (index = 0; index < TIC_PERIOD_MAX; index++)
      {
        if (teleinfo_contract_db.arr_period[index].valid || (teleinfo_conso_wh.index[index] > 0))
        {
          ...
          if (TeleinfoContractGetPeriodHP () == 0) strcat_P (str_text, PSTR (" tico75"));
  ...
  if (teleinfo_conso.enabled)
  {
    ...
    // header
    WSContentSend_P (PSTR ("<div class='tic ticb'><div class='tic70l'>%s"), PSTR ("Consommation"));
  ...
  if (teleinfo_prod.enabled)
  {
    ...
    WSContentSend_P (PSTR ("<div class='tic ticb'><div class='tic48l'>%s</div>"), PSTR ("Revente"));
  ...
  // cacsi contract
  else if (teleinfo_prod.cacsi)
  {
    // display
    WSContentSend_P (PSTR ("<div class='sec'>\n"));
    WSContentSend_P (PSTR ("<div class='tic ticb'><div class='tic48l'>%s</div>"), PSTR ("Surplus"));
    WSContentSend_P (PSTR ("<div class='tic40r'>")); 
    if (teleinfo_prod.papp > 0) WSContentSend_P (PSTR ("%d à %d"), teleinfo_prod.papp - 115, teleinfo_prod.papp + 115);
      else WSContentSend_P (PSTR ("0"));
    WSContentSend_P (PSTR ("</div><div class='tic02'></div><div class='tic10l'>VA</div>")); 
  ...
  {
    // separator and header
    GetTextIndexed (str_color, sizeof (str_color), TeleinfoContractGetPeriodLevel (), kTeleinfoLevelIcon);  
