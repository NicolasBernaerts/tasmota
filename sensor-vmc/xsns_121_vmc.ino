...
void CmndVmcMode ()
{
  ...
  if ((XdrvMailbox.data_len > 0) && (XdrvMailbox.payload < VMC_MODE_MAX))
  {
...
{
  // set activation state
  if ((XdrvMailbox.data_len > 0) && (XdrvMailbox.payload > 0) && (XdrvMailbox.payload < 50)) 
  {
...
void VmcWebMainButton ()
{
  ...
  WSContentSend_P (PSTR ("<option value='%u'>-- Select mode --</option>\n"), UINT8_MAX);
...
void VmcWebSensor ()
{
  ...
  GetTextIndexed (str_text, sizeof (str_text), vmc_config.mode, kVmcModeLabel);
  WSContentSend_P (PSTR ("{s}%s{m}%s{e}"), D_VMC_MODE, str_text);

  // if automatic mode, add target humidity
  if (vmc_config.mode == VMC_MODE_AUTO) WSContentSend_P (PSTR ("{s}%s{m}%u %%{e}"), D_VMC_TARGET, vmc_config.target);
}
