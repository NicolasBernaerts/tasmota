...
void CmndSensorTemperatureValue ()
{
  ...
  ResponseCmndFloat (SensorReadTemperature (timeout), 1);
}
...
void CmndSensorHumidityValue ()
{
  ...
  ResponseCmndFloat (SensorReadHumidity (timeout), 1);
}
...
{
  uint32_t timeout = SENSOR_TIMEOUT_MOVEMENT;

  if (XdrvMailbox.payload > 0) timeout = (uint32_t)XdrvMailbox.payload; 

  ResponseCmndNumber ((int)SensorReadMovement (timeout));
}
...
float SensorReadTemperature () { return SensorReadTemperature (SENSOR_TIMEOUT_TEMPERATURE); }
float SensorReadTemperature (uint32_t timeout)
{
  uint32_t index;
  float    value = NAN;
...
// read humidity level
float SensorReadHumidity () { return SensorReadHumidity (SENSOR_TIMEOUT_HUMIDITY); }
float SensorReadHumidity (uint32_t timeout)
{
  float value = NAN;
...
bool SensorReadMovement () { return SensorReadMovement (SENSOR_TIMEOUT_MOVEMENT); }
bool SensorReadMovement (uint32_t timeout)
{
  bool result = false;
