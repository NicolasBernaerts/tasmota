  ...
  if (mqtt_humidity.subscribed)
  {
    ...
    if (strcmp (pstr_topic, XdrvMailbox.topic) == 0)
    {
      ...
      if (pstr_value != nullptr)
      {
        // extract humidity
        pstr_value++;
        humidity = strtof (pstr_value, &pstr_result);

        // if humidity has been read
        ...
        if (data_handled)
        {
          ...
          ext_snprintf_P (str_buffer, sizeof(str_buffer), PSTR ("%02_f"), &humidity);
          AddLog (LOG_LEVEL_INFO, PSTR ("MQT: Humidity is %s %%"), str_buffer);
        }
      }
    }
  }
  ...
  { 
   ...
   case FUNC_MQTT_DATA:
    ...
    case FUNC_WEB_ADD_BUTTON:
      WSContentSend_P (PSTR ("<p><form action='%s' method='get'><button>Configure Remote Humidity</button></form></p>"), D_PAGE_HUMIDITY);
      break;
#endif  // USE_WEBSERVER
  }
