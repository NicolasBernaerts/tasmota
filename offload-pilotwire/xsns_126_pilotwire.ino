...
void PilotwireMovementUpdateDetection ()
{
  ...
  mvt_detected = SensorReadMovement ();
  ...
  if (mvt_detected)
  {
    ...
    if ((pilotwire_status.device_mode == PILOTWIRE_STATUS_ECO) || (pilotwire_status.device_mode == PILOTWIRE_STATUS_NOFROST)) PilotwireSetStatusMode (PILOTWIRE_STATUS_COMFORT);
  }
...
void PilotwireEverySecond ()
{
  ...
  pilotwire_mvt.present = (SENSOR_SOURCE_MAX != SensorSourceMovement ());

  // update temperature and update JSON if temperature has changed of more than 0.1°C
  pilotwire_temperature.current = SensorReadTemperature ();
...
void PilotwireInit ()
{
  ...
  pilotwire_mvt.present = PinUsed (GPIO_SWT1, SENSOR_MOVEMENT_INDEX);
